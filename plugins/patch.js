  ...
  async (message, args) => {
    try {
      ...
      if (url.toLowerCase().includes("https")) {
        ...
        return await message.send("`TANGIRO_KAMADO- UPDATE WAS SUCCESSFULL ✅`");
      } else {
...
${menuThemeFooter}                         
*┌┤✑  Thanks for Choosing TANGIRO_KAMADO
│└────────────┈ ⳹        
│*©2024-2099 Viciouswrld🕴️*
└─────────────────┈ ⳹*
${readmore}`;
  ...
  async (_0x22514a) => {
    try {
      let _0x20ed34 = (
        ...
        "/" +
        formatp(os.totalmem()) +
        "\n\n *🧑‍💻 :*  TANGIRO_KAMADO ɪꜱ ɴᴏᴡ Aᴠᴀɪʟᴀʙʟᴇ\n\n" +
        readmore +
        "\n╭──❰ *ALL MENU* ❱\n│🏮 Lɪꜱᴛ\n│🏮 Cᴀᴛᴇɢᴏʀʏ\n│🏮 Hᴇʟᴘ\n│🏮 Aʟɪᴠᴇ\n│🏮 Uᴘᴛɪᴍᴇ\n│🏮 Wᴇᴀᴛʜᴇʀ\n│🏮 Lɪɴᴋ\n│🏮 Cᴘᴜ\n│🏮 Rᴇᴘᴏꜱɪᴛᴏʀʏ\n╰─────────────⦁"
      ).trim();
...
astro_patch.smd(
  ...
  async (context) => {
    ...
    await context.send(
      `*TANGIRO_KAMADO ping is: ${pingTime} ᴍs*`,
      { edit: messageKey },
      "",
      context
    );
    ...
    try {
      ...
      const designs = [
        async () => {
          ...
          const captionText = `TANGIRO_KAMADO 1.𝟶.𝟶 ᴘᴀᴛᴄʜ\n\n*Ping:* ${pingSeconds} seconds${quoteText}\n\nTANGIRO_KAMADO`;
...
smd(
  ...
  async (m) => {
    try {
      ...
      const message = `*TANGIRO_KAMADO ʀᴜɴɴɪɴɢ sɪɴᴄᴇ:* ${uptimeHours}h ${uptimeMinutes}ᴍ ${uptimeSeconds}s\n \t_ʜᴇʀᴇ's ᴍᴏʀᴇ ɪɴғᴏ_\n*ʀᴀᴍ ᴜsᴀɢᴇ:* ${ramUsage.toFixed(2)} MB\n*ᴄᴘᴜ ɴᴀᴍᴇ:* ${cpuModel}`;
      const contextInfo = {
        isForwarded: true,
        forwardingScore: 999,
        title: "TANGIRO_KAMADO Running Since",
        body: message,
        footerText: "TANGIRO_KAMADO 2024",
    ...
    try {
      ...
      if (!isNaN(_0x56b5d3)) {
        if (
          ...
          _0x1eb88a[1][_0x210c92(362)]("TANGIRO_KAMADO_FANCY_TEXT")
        ) {
